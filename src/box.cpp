...
void Box::collision() {
    //Gravity
    v.y.raw += GRAV;
    size -= Vector(1, 1); //This will be undone later.
    ...
    int16_t iTo = (nextPos.x.whole + size.x) / level->getTileWidth() ;
    int16_t jFrom = nextPos.y.whole / level->getTileWidth();
    int16_t jTo = (nextPos.y.whole + size.y) / level->getTileWidth();
    ...
    rect = {iFrom * w, jFrom * w, (iTo - iFrom + 1) * w, (jTo - jFrom + 1) * w};
    ...
    nextPos = pos;
    // nextPos = pos + v + Vector(0, 2 * (standing & STANDING_BOTTOM_bm));
   
    //Top right collision
    if(!level->isPassable(iTo, jFrom)) {
    ...
    //Top left collision
    if(!level->isPassable(iFrom, jFrom)) {
